...
try:
    ...
    try:
        if root:
            subprocess.run(["nmap", "-T3", "-sC", "-sV", "-osscan-guess", "-O", target, "-oN", f"./{target}/nmap-{target}.txt"], check=True)
        else:
            subprocess.run(["nmap", "-T3", "-sC", target, "-oN", f"./{target}/nmap-{target}.txt"], check=True)
    except:
        print(f"\n[-] Nmap error\n")
